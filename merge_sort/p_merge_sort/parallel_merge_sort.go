...
func PMergeSort(A []int64, leftIndex int64, rightIndex int64, B []int64, s int64, depth int){
	n := rightIndex - leftIndex + 1
	if n == 1{
		B[s] = A[leftIndex]
	}else{
		...
		c := make(chan bool)
		go func(c chan bool){
			PMergeSort(A, leftIndex, mid1, T, 1, depth+1)
			c <- true
		}(c)
		PMergeSort(A,mid1 + 1, rightIndex, T, mid2 + 1,depth+1)
		<-c
